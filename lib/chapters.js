...
function makeChapterList(chapterArr) {
  if (!chapterArr) {
    chapterArr = [];
  }
  let str = '<ul class="chapter-area">';
  chapterArr.forEach(function (c) {
    ...
    str += '<a href="' + chapterPath + '" target="_blank">' + c[Object.keys(c)[0]] + '</a>';
    str += '</li>';
  });
  str += '</ul>';
  return str;
...
function chapters(option) {
  ...
  if (isChaptersExists) {
    ...
    */
    ...
    option.chapterList = makeChapterList(option.chapters);
  ...
  try {
    ...
    log.error('[ERROR] cannot find your document source directory');
    process.exit(1);
    // throw e;
  }
