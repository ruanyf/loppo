...
function makeBreadcrumb(root, optionObj) {
  ...
  optionObj.breadcrumbOrigin.forEach(function (c) {
    ...
    } else if (bcPath.substr(-1) === path.sep) {
      bcPath += 'index.html';
    }
    const str = '<a href="'
      + optionObj.relative_root_path
...
function makeChapterList(root, optionObj) {
  ...
  chapterArr.forEach(function (c, i) {
    ...
    } else if (chapterPath.substr(-1) === path.sep) {
      chapterPath += 'index.html';
    }

    // embedding chapter
    const chapterPathArr = chapterPath.split(path.sep);
    ...
    if (level === current_level && chapterPathOrigin.split(path.sep)[level] === '') {
      str += '</ul>';
    }

    let isFirstLevelDir = false;
    let isCurrentFirstLevelDir = false;
    if (level === 1 && chapterPathOrigin.split(path.sep)[level] === '') {
    ...
    if (
      level > current_level
      || (level === current_level && chapterPathOrigin.split(path.sep)[level] === '')
    ) {
      let collapseClass = '';
...
function markdownRender(root, optionObj) {
  function highlight(str, lang) {
    ...
    return '<pre class="hljs"><code>' + (markdownIt.utils ? markdownIt.utils.escapeHtml(str) : str) + '</code></pre>';
  }
