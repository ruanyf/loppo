...
function makePreviousPage(root, optionObj) {
  ...
  if (pp === null) {
    ...
    optionObj.previous_page = '<a class="previouse-article-link" href="'
...
function makeNextPage(root, optionObj) {
  ...
  if (np === null) {
    ...
    optionObj.next_page = '<a class="next-article-link" href="'
      + optionObj.relative_root_path
      + np_path
      + '">Next &raquo;</a>';
  }
...
function makeBreadcrumbOrigin(root, optionObj) {
  ...
  if (root !== '/') {
    ...
    for (let i = 0; i < pathArr.length - 1; i++) {
      ...
      for (let m = 0; m < optionObj.chapters.length; m++) {
...
function makeChapterList(root, optionObj) {
  ...
  chapterArr.forEach(function (c, i) {
    let chapterPath = Object.keys(c)[0];
    ...
    if (level > current_level) {
      str += '<ul class="chapter-level-' + level + '">';
    }
