...
function makeIndex(root, optionObj) {
  ...
  if (root !== '/') {
    ...
    throw new Error('Cannot find the directory in Chapters!');
  }
  ...
  if (root === '/') {
    ...
    chapters.forEach(p => {
      ...
      if (relativePath.substr(-3).toLowerCase() === '.md') {
        pageHtml = relativePath.substr(0, pagePath.length - 3) + '.html';
      } else {
  ...
  } else if (rootPosition === 0) {
    optionObj.previous_page = { 'index.md': 'Home' };
  } else {
    optionObj.previous_page = optionObj.chapters[rootPosition - 1];
  }

  // create option.next_page
  if (!optionObj.chapters.length) {
    ...
    if (optionObj.chapters[rootPosition + 1]) {
      optionObj.next_page = optionObj.chapters[rootPosition + 1];
    } else {
      optionObj.next_page = null;
    }
  }
